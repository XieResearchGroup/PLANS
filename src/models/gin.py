...
class GraphCNN(tf.keras.Model):
    def __init__(
        ...
        """
            num_layers: number of layers in the neural networks (INCLUDING the input
                layer)
            ...
            learn_eps: If True, learn epsilon to distinguish center nodes from
                neighboring nodes. If False, aggregate neighbors and center nodes
                altogether.
    ...
    def __preprocess_neighbors_maxpool(self, batch_graph):
        ...
        # compute the maximum number of neighbors within the graphs in the current
        # minibatch
        ...
        for i, graph in enumerate(batch_graph):
            ...
            for j in range(len(graph.neighbors)):
                ...
                # Add center nodes in the maxpooling if learn_eps is False, i.e.,
    ...
    def __preprocess_neighbors_sumavepool(self, batch_graph):
        ...
        # Add self-loops in the adjacency matrix if learn_eps is False, i.e., aggregate
        # center nodes and neighbor nodes altogether.
        if not self.learn_eps:
            ...
            # Adding connections from self-connections to the list of other
            # connections specified
            Adj_block_idx = tf.concat([Adj_block_idx, self_loop_edge], 1)
            Adj_block_elem = tf.concat(
                [Adj_block_elem, elem], 0
            )  # Total number of connections + number of nodes

        Adj_block_idx = tf.cast(tf.transpose(Adj_block_idx), tf.int64)
        Adj_block = tf.SparseTensor(
            indices=Adj_block_idx,
            values=Adj_block_elem,
            dense_shape=[start_idx[-1], start_idx[-1]],
        )
        return Adj_block

    def __preprocess_graphpool(self, batch_graph):
        # create sum or average pooling sparse matrix over entire nodes in each graph
        # (num graphs x num nodes)
        start_idx = [0]
        ...
        for i, graph in enumerate(batch_graph):
            ...
            # idx will be [[0,0],[0,1]...[0,218<end of the 1st graph>],[1,219<end of the
            # first graph + 1>]...]
            idx.extend([[i, j] for j in range(start_idx[i], start_idx[i + 1], 1)])

        elem = tf.constant(elem)
        graph_pool = tf.SparseTensor(
            indices=idx, values=elem, dense_shape=[len(batch_graph), start_idx[-1]]
        )
        # graph_pool is a diagonal matrix of ones, where the ones are rows corresponding
        # to the length of each graph.
    ...
    def call(self, batch_graph):
        ...
        for layer in range(self.num_layers - 1):
            if self.neighbor_pooling_type == "max" and self.learn_eps:
                ...
                not self.neighbor_pooling_type == "max" and self.learn_eps
            ):  # This is the one that triggers for the reddit dataset
