...
class HMLC(Model):
    ...
    def call(self, inputs, training=None):
        ...
        if training:
            ...
            final_out = tf.sigmoid(final_out)
        else:
            final_out = tf.nn.sigmoid(global_local_sum)
        # else:
            # final_out = tf.keras.backend.sigmoid(global_local_sum)
            # final_out = tf.sigmoid(global_local_sum)
